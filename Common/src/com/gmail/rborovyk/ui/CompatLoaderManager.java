...
import com.gmail.rborovyk.common.MyHCSparseArray;
...
public class CompatLoaderManager extends LoaderManager{
    ...
    final MyHCSparseArray<LoaderInfo> mLoaders = new MyHCSparseArray<LoaderInfo>();
    ...
    final MyHCSparseArray<LoaderInfo> mInactiveLoaders = new MyHCSparseArray<LoaderInfo>();
    ...
    boolean mOuterRetaining;
    
    public static void enableDebugLogging(boolean enabled) {
        DEBUG = enabled;
    }
    
    public CompatLoaderManager(Activity activity) {
        mActivity = activity;
        mStarted = false;
    }
    ...
    //-------------------------------------------------------------------------------------------------
    public static CompatLoaderManager onCreate(Activity activity) {
    	CompatLoaderManager lm = (CompatLoaderManager)
    			activity.getLastNonConfigurationInstance();
        if (null == lm) {
            lm = new CompatLoaderManager(activity);
        }else{
        	lm.updateActivity(activity);
        }
        return lm;
    }
    ...
    public Object retainInstance() {
    	return mRetaining ? this : null;
    }
    
    void doReallyStop(boolean retaining) {
        if (!mReallyStopped) {
            mReallyStopped = true;
            mOuterRetaining = retaining;
            mHandler.removeMessages(MSG_REALLY_STOPPED);
    ...
    private LoaderInfo createLoader(int id, Bundle args,
    		CompatLoaderManager.LoaderCallbacks<Object> callback) {
        LoaderInfo info = new LoaderInfo(id, args,  (CompatLoaderManager.LoaderCallbacks<Object>)callback);
    ...
    public <D> Loader<D> initLoader(int id, Bundle args, CompatLoaderManager.LoaderCallbacks<D> callback) {
        if (mCreatingLoader) {
        ...
        if (info == null) {
            ...
            info = createAndInstallLoader(id, args,  (CompatLoaderManager.LoaderCallbacks<Object>)callback);
            if (DEBUG) Log.v(TAG, "  Created new loader " + info);
        } else {
            if (DEBUG) Log.v(TAG, "  Re-using existing loader " + info);
            info.mCallbacks = (CompatLoaderManager.LoaderCallbacks<Object>)callback;
        }
     ...
     */
    @SuppressWarnings("unchecked")
    public <D> Loader<D> restartLoader(int id, Bundle args, CompatLoaderManager.LoaderCallbacks<D> callback) {
        if (mCreatingLoader) {
            throw new IllegalStateException("Called while creating a loader");
        }
    ...
    final class LoaderInfo implements Loader.OnLoadCompleteListener<Object> {
        final int mId;
        final Bundle mArgs;
        CompatLoaderManager.LoaderCallbacks<Object> mCallbacks;
        ...
        public LoaderInfo(int id, Bundle args, CompatLoaderManager.LoaderCallbacks<Object> callbacks) {
            mId = id;
            mArgs = args;
